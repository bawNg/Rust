...
cp -f Oxide/Oxide.Core/bin/Release/Oxide.Core.dll \
Oxide/Oxide.Ext.CSharp/bin/Release/Oxide.Ext.CSharp.dll \
Oxide/Oxide.Ext.JavaScript/bin/Release/Oxide.Ext.JavaScript.dll \
Oxide/Oxide.Ext.Lua/bin/Release/Oxide.Ext.Lua.dll \
Oxide/Oxide.Ext.Python/bin/Release/Oxide.Ext.Python.dll \
Oxide/Oxide.Ext.Unity/bin/Release/Oxide.Ext.Unity.dll \
...
cd Oxide/Dependencies || die_with "Failed to change to dependencies directory!"
...
mkdir -p $HOME/snapshots/Rust || die_with "Failed to create snapshot version directory!"
zip -vr9 $HOME/snapshots/Rust/OxideRust.zip . || die_with "Failed to archive snapshot files!"

echo "Adding, committing, and pushing to snapshots branch"
cd $HOME/snapshots || die_with "Failed to change to snapshots directory!"
git add -f . && git commit -m "Oxide 2.0.$TRAVIS_BUILD_NUMBER for Rust" || die_with "Failed to add and commit files with git!"
